# Copyright 2019 Brian Groenke. All Rights Reserved.
...
# ==============================================================================
"""Modified implementation of Tensorflow Cluster Resolvers for Kubernetes."""
...
class KubernetesClusterResolver(ClusterResolver):
  ...
  def cluster_spec(self, require_pod_running=False):
    ...
    for tf_job in self._job_to_label_mapping:
      ...
      for selector in self._job_to_label_mapping[tf_job]:
        ret = client.list_namespaced_pod(self._namespace, label_selector=selector)
        selected_pods = []
        ...
        for pod in sorted(ret.items, key=lambda x: x.metadata.name):
          if not require_pod_running or pod.status.phase == 'Running':
